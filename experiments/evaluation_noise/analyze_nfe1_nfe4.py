...
out = {"protocol": {"R": R, "seeds": list(SEEDS), "arms": [1, 4],
...
def load(seed, nfe):
    S = []
    for r in (1, 2, 3):
        f = os.path.join(R_DIR, f"r0_s{seed}_nfe{nfe}_r3rep{r}.json")
        d = json.load(open(f))
...
print(f"{'seed':>5s} {'p(NFE1)':>9s} {'p(NFE4)':>9s} {'Delta':>9s} {'95% CI':>22s}")
per_seed, P = {}, {}
for s in SEEDS:
    eps, S1 = load(s, 1); _, S4 = load(s, 4)
    ...
    per_seed[s] = {"p_nfe1": float(p1.mean()), "p_nfe4": float(p4.mean()),
                   "delta": float(d.mean()), "ci": [float(lo), float(hi)],
                   "eff_successes_nfe1": float(p1.sum()), "eff_successes_nfe4": float(p4.sum()),
                   "n_episodes": len(eps)}
...
for s in SEEDS:
    ...
    sens[str(s)] = {"nfe1": {"robust_success": a, "robust_fail": b, "sensitive": c},
                    "nfe4": {"robust_success": a4, "robust_fail": b4, "sensitive": c4}}
    print(f"  seed {s}: NFE1 robust+={a:3d} robust-={b:3d} sensitive={c:3d} | "
...
out["secondary"] = sec
json.dump(out, open("experiments/evaluation_noise/nfe1_nfe4_analysis.json", "w"), indent=2)
print("\nwrote nfe1_nfe4_analysis.json")
