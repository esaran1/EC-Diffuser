...
CKPT_DIR = ("/home/jren313/ecdiff-seed43-7506ce48/data/panda_push/flow/"
            "3C_dlp_adalnpint_randcolor_H5_T4_seed43")
CKPT_SHA = "c8e00eadfed9b8a0b54c5423864457e5330434b748c50f820cb7d7a0328ac826"
OUT_DIR = "experiments/isaacgym_control/seed43"
...
def main():
    ...
    for task, cubes, set_paths, expect in TASKS:
        ...
        for nfe in NFES:
            out = f"{OUT_DIR}/seed43_{task}_H{HORIZON}_flow_nfe{nfe}.json"
            if os.path.exists(out):
                print(f"[skip] {out}", flush=True)
                continue
            experiment = utils.load_diffusion(
                "data", args.dataset,
                "flow/3C_dlp_adalnpint_randcolor_H5_T4_seed43",
            ...
            summary = {
                "training_seed": 43, "label": f"flow_nfe{nfe}", "arm": "flow",
